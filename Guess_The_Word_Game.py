...
def get_guess():
    guesses_left = 10
    secret_word = random.choice(words).lower()
    current_dashes_state = "-" * len(secret_word)
    while guesses_left > 0:
        ...
        if not guess in secret_word:
            ...
            current_dashes_state = update_dashes(secret_word, current_dashes_state, guess)
            if guesses_left == 0:
                break
            print(current_dashes_state)
            print(f"{guesses_left} incorrect guesses left.")
            continue
        if guess in secret_word and guess.islower() and len(guess) ==1:
            print("That letter is in the secret word!")
            guesses_left = guesses_left
            current_dashes_state = update_dashes(secret_word, current_dashes_state, guess)
            print(current_dashes_state)
            if current_dashes_state == secret_word:
                break
            print(f"{guesses_left} incorrect guesses left.")
            continue
    return current_dashes_state

def update_dashes(secret_word, current_dashes_state, guess):
